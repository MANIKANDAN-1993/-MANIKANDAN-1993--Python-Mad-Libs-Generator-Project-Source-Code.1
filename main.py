...
def Story1(win):
    def final(tl: Toplevel, name, sports, City, playername, drinkname, snacks):
        ...
        We really enjoyed! We are looking forward to going again and enjoying.'''
...
def Story2(win):
    def final(tl: Toplevel, profession, noun, feeling, emotion, verb):
        text = f'''
            One day me and my friend {name} decided to play a {sports} game in {City}.
            But we were not able to play. So, we went to the game and watched our favorite player {playername}.
            We drank {drinkname} and also ate some {snacks}.
    ...
    SubmitButton = Button(NewScreen, text="Submit", background="Blue", font=('Times', 12),
                          command=lambda: final(NewScreen, Profession.get(), Noun.get(), Feeling.get(), Emotion.get(),
                                               Verb.get()))
...
Story2Button = Button(Screen, text='Ambitions', font=("Times New Roman", 13), command=lambda: Story2(Screen),
                      bg='Blue')
